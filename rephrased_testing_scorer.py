#!/usr/bin/env python3
"""
Scores flat email files in Rephrased Testing/ with ChatGPT and writes/resumes
results in Rephrased_testing.csv.

Each .txt file directly inside Rephrased Testing/ is sent to ChatGPT, which
returns a maliciousness score:
  0 = completely benign, 100 = completely malicious.

CSV columns: filename, score

IMPORTANT:
- If Rephrased_testing.csv already exists, previous scores are preserved.
- Only missing file scores are sent to the API.
- This allows the scorer to resume after interruption and prevents re-scoring
  existing files.

Usage:
  python3.12 -B rephrased_testing_scorer.py
  python3.12 -B rephrased_testing_scorer.py --api-key sk-... --model gpt-4o-mini
  python3.12 -B rephrased_testing_scorer.py --overwrite-existing
...
INPUT_DIR = ROOT / "Rephrased Testing"
OUTPUT_COLUMNS = ["filename", "score"]
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Score flat files inside Rephrased Testing with ChatGPT"
    )
    ...
    parser.add_argument(
        ...
        help="Input folder containing flat .txt files",
    )

    parser.add_argument(
        "--output",
        default="Rephrased_testing.csv",
    ...
    scores = {} if args.overwrite_existing else load_existing_scores(out_path)

    if args.overwrite_existing:
        print("Overwriting existing scores.")
    elif scores:
        print(f"Loaded {len(scores)} existing score(s) from {out_path}")
    ...
    for pos, email_file in enumerate(files, start=1):
        ...
        for attempt in range(3):
            try:
                ...
                print(
                    f"  retry {attempt + 1}/3 for {filename}: {exc}",
                    file=sys.stderr,
                )
                time.sleep(2)
