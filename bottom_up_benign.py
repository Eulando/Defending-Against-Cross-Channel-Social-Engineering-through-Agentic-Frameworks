#!/usr/bin/env python3
"""
Generates benign multi-email scenarios bottom-up (as opposed to
top_down_benign.py which works top-down): the four individual emails are
created first, then the consolidated email is derived from them.

Every scenario gathers pieces of information about a single target person from
four different receivers, all in email format. For each set K:
...
def build_message(rng, rec, params):
    """Build a single email body bottom-up from the shared scenario params."""
    ...
    context = (
        f"We are putting together {params['target_name']}'s {params['event']}, "
        f"and I could use your help with a couple of details."
    )
    request = rng.choice(requests).format(
        first=params["target_first"], p1=params["p1"], p2=params["p2"],
        event=params["event"], gift=params["gift"], code=params["code"],
        room=params["room"], guests=params["guests"],
...
def generate(idx: int) -> None:
    ...
    # Bottom-up: build the four individual emails first, then the combined one.
    for n, rec in enumerate(recs, start=1):
        body = build_message(rng, rec, params)
        dest = ROOT / f"ind{n}" / f"data{idx}.txt"
        dest.write_text(body + "\n")

    combined_email = build_combined(recs, params, coordinator)
    dest = ROOT / "combined" / f"data{idx}.txt"
    dest.write_text(combined_email + "\n")
