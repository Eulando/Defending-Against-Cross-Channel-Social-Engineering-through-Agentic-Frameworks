...
API and save them into "7_emails_2nd_prompt".
...
Usage:
  python3.12 generate_7_emails_2nd_prompt.py
  python3.12 generate_7_emails_2nd_prompt.py --api-key sk-...
  python3.12 generate_7_emails_2nd_prompt.py --model gpt-5.6-sol
  python3.12 generate_7_emails_2nd_prompt.py --overwrite
...
OUTPUT_DIR = ROOT / "7_emails_2nd_prompt"

PROMPT = (
    "I am working on generating a dataset for cybersecurity for cross channel "
    "social engineering attacks. As a basic framework, generated malicious data "
    "were scored using open ai api keys, however we were wondering if "
    "maliciousness could be affected by adding sentences and/or rephrasing the "
    "sentence structure while keeping the original information intact. Please "
    "apply the previous modifications to the emails."
)
...
You may add brief sentences that supplement the context, but do not add
anything that entirely changes the nature, purpose, sender, recipient, or
requested information in the email. Do not add explicit cautious warnings about
sending information over email, verifying email safety, secure channels, or
