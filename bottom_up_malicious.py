#!/usr/bin/env python3
"""
Generates bottom-up malicious multi-email scenarios in the shared dataset
layout.

For each set K:
  - ind1..ind4/dataK.txt -> four seemingly benign internal emails. Each one
                            asks for one personal or sensitive detail about the
                            same fictional person.
  - combined/dataK.txt   -> one malicious email that asks a single recipient
                            to collect all four sensitive details at once.

The bottom-up order is intentional: the four individual emails are created
first, and the combined email is built from those four requested pieces.

By default all sets 500..599 are generated; pass one or two numbers to create
a subset, e.g.
  python bottom_up_MAL.py 500 599
  python bottom_up_MAL.py 525
"""
...
def pick_balanced(rng, pool, counter):
    """Pick the least-used item, with random tie-breaking."""
...
def render_piece(piece_template, params):
    return (
        piece_template.replace("{victim}", params["victim_name"])
...
def build_individual_email(rng, rec, params):
    """Build one benign-looking email asking for one sensitive detail."""
    recipient_name, recipient_role, requests, piece_template = rec
...
def build_combined_email(individual_emails, params, coordinator):
    """Build one malicious email from the four already-created requests."""
    coordinator_first = coordinator[0].split()[0]

    tasks = []
    for idx, email in enumerate(individual_emails, start=1):
...
def generate(idx):
    ...
    for number, rec in enumerate(recs, start=1):
        email = build_individual_email(rng, rec, params)
        individual_emails.append(email)
        dest = ROOT / f"ind{number}" / f"data{idx}.txt"
        dest.write_text(email["body"] + "\n", encoding="utf-8")

    combined = build_combined_email(individual_emails, params, coordinator)
